...
def get_performance(true_matrix, pred_matrix) :
    ...
    plt.xlabel("M")
    plt.ylabel("Recall")
    plt.title("Performance of CDL in dense setting")
    plt.legend()
    plt.savefig("./data/citeulike-a/CDL_dense.png")

    df = pd.DataFrame(accuracy, columns=["accuracy"])
    df.to_csv("./data/citeulike-a/CDL_dense_accuracy.csv")                
...
if __name__ == "__main__" :
    ...
    # Get Performance
    get_performance(rating_matrix_true, rating_matrix_pred)